...
@dataclass
class mHubertPretrainingConfig(FairseqDataclass):
    data: str = field(default=MISSING, metadata={"help": "path to data directory"})
    fine_tuning: bool = field(
        default=False, metadata={"help": "set to true if fine-tuning Hubert"}
    )
...
@register_task("mhubert_pretraining", dataclass=HubertPretrainingConfig)
class mHubertPretrainingTask(FairseqTask):

    cfg: mHubertPretrainingConfig
    ...
    def __init__(
        ...
        logger.info(f"current directory is {os.getcwd()}")
        logger.info(f"mHubertPretrainingTask Config {cfg}")
    ...
    @classmethod
    def setup_task(
        cls, cfg: mHubertPretrainingConfig, **kwargs
    ) -> "mHubertPretrainingTask":
        return cls(cfg)
